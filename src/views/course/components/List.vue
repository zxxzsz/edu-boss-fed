...
<template>
  <div class="course-list">
    <el-card>
      ...
      <el-form :inline="true" ref="form" label-position="left" :model="filterParams">
        ...
        <el-form-item>
          ...
          <el-button type="primary" :disabled="isLoading" @click="handleFilter">查询
          </el-button>
        </el-form-item>
      </el-form>
    </el-card>
    <el-card>
      <div slot="header">
        <span>查询结果：</span>
        <el-button style="float: right; margin-top: -10px" type="primary" @click="
          $router.push({
            name: 'course-create',
          })
        ">添加课程
      ...
      <el-pagination background layout="prev, pager, next" :total="totalCount" :disabled="isLoading"
        :current-page="filterParams.currentPage" :page-size="filterParams.pageSize"
...
export default {
  ...
  methods: {
    ...
    async onStateChange (course) {
      ...
      const { data } = await changeState({ courseId: course.id, status: course.status })
      if (data.code === '000000') {

    ...
    <el-card>
      <div slot='header'>
        ...
        <el-steps :active='activeStep' simple>
          <el-step v-for='(item, index) in steps' :key='item.id' @click.native='activeStep = index' :title='item.title'
            :icon='item.icon' />
        </el-steps>
      </div>
      <!-- 步骤对应的表单结构 -->
      <el-form label-width='80px'>
        ...
        <div v-show='activeStep === 0'>
          ...
          <el-form-item label='课程概述'>
            ...
            <el-input v-model='course.previewSecondField' placeholder='概述2' style='width: 49%; min-width: 300px' />
          </el-form-item>
          ...
          <el-form-item label='课程排序'>
            <el-input-number v-model='course.sortNum' label='描述文字' controls-position='right' />
          </el-form-item>
        </div>
        <!-- 课程封面 -->
        <div v-show='activeStep === 1'>
          <!-- 封装为图片上传组件 -->
          <course-image v-model='course.courseListImg' label='课程封面' :limit='3' />
          <course-image v-model='course.courseImgUrl' label='解锁封面' />
        </div>
        <!-- 销售信息 -->
